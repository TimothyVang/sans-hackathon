...
> ground truth (nitroba 5/5 = 100%, reproducible from `docs/sample-run/nitroba`; NIST 7/14 = 50% (up from 7%);
...
are not identical: the local run surfaces 27 findings at 7/14 = 50% recall while the older
SIFT-VM reference surfaces 19 findings at 5/14 = 36% — both ed25519-signed and SUSPICIOUS, not
exact mode parity (re-score the SIFT reference first; see `docs/sample-run/README.md`), and
