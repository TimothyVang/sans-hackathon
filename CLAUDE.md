...
1. **`docs/superpowers/specs/2026-04-23-find-evil-automation-master-design.md`** — master design. Defines the 4-subsystem decomposition and the 4 differentiators (M1 leaderboard, M2 crypto chain-of-custody, M3 MCP App widgets, M4 ACH competing-hypothesis agents).
...
7. **Per-subsystem specs** (in `docs/superpowers/specs/`):
...
8. **Implementation plans** (in `docs/superpowers/plans/`) — one per spec, each step written as a TDD checkbox with the exact failing test → implement → commit sequence.
