...
- **Rust MCP tool count is 12, not 11.** Spec #2 §6 enumerates 11; we ship `vol_psscan` as the 12th for DKOM cross-validation against `vol_pslist`. The pair is deliberately redundant (active-list walk vs pool-memory signature scan); divergence between them IS the T1014/Rootkit finding. Don't fold them.
