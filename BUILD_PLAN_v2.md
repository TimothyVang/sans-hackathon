...
> **Superseded sections (read this first):** Sections that name `services/agent/graph.py`, `services/agent/api.py`, `services/agent/cli.py`, `services/agent/supervisor.py`, or `services/agent/specialists/` describe the pre-A2 architecture. Amendment A2 (2026-04-25, see `docs/superpowers/specs/2026-04-25-amendment-a2-claude-code-primary-interface.md`) drops the custom Python orchestrator entirely — Claude Code IS the orchestrator, and the M2 + M4 stacks ship as a Python MCP server (`services/agent_mcp/`). The DFIR fundamentals, rubric analysis, demo beat structure (§9), risk register (§11), and research questions (§12) all stand. Treat any §10 "Week-1 skeleton" file path that names a dropped module as historical context, not as guidance.
>
> **Amendment A3 (2026-04-26, see `docs/superpowers/specs/2026-04-26-amendment-a3-agent-army-and-dashboard.md`) un-defers `apps/web/`** as a NES.css live dashboard with 5 pixel-art sprites (Pool A/B/Verifier/Judge/Correlator) tailing the audit JSONL over WebSocket. §6 ("Web UI component contract") is back in scope — but with an aesthetic shift from shadcn/ui to NES.css; treat §6's component names as starting-point inventory, not final API. A3 also adds three new tools to `services/agent_mcp/` — `memory_remember` + `memory_recall` (Hermes-pattern FTS5 cross-case memory, restoring the L3 capability §3 deferred under A2) and `pool_handoff` (IBM Agent Communication Protocol envelope, recorded into the audit JSONL). `apps/mcp-widgets/` remains deferred per A2 §2.1.
