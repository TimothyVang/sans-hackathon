...
# DFIR tool flakiness must not stall the swarm's PR pipeline.
...
# Hayabusa 2.x (AGPL-3.0, subprocess only) from the official release
# tarball. Pinned by version + SHA for reproducibility.
...
RUN curl -fsSL \
 ...
 && find /opt/hayabusa -maxdepth 2 -name 'hayabusa*' -type f -executable \
       -exec ln -sf {} /usr/local/bin/hayabusa \; \
