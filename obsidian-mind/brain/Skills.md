...
## obsidian-mind curation commands (Slash Commands)
...
If QMD is installed (`npm install -g @tobilu/qmd`, under Node 22), the vault has semantic search. Every command takes `--index <name>` where `<name>` is `vault-manifest.json`'s `qmd_index` field (here: `verdict-memory`):
